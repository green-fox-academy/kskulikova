...
class Updoot {
  ...
  request(method, path, requestData = {}, validatorFn) {
    ...
    try {
      ...
      if (validatorFn && validatorFn(error.responseJSON || {}, error)) {
        return this.addAlert(`${endpointName}`, 'success',
            `correct error response - ${JSON.stringify(error)}`);
      } else {
        return this.addAlert(`${endpointName}`, 'danger',
  ...
  onDoublingClick() {
    // no error expected
    await
    this.request('get', '/doubling');

    // expected 200, with specific response
    await
    this.request('get', '/doubling', {input: 5}, data = > {
      console
      .info('Validating, received must be equal to 5, result must be 10');
    return 'received' in data && 'result' in data && data['received'] == 5
        && data['result'] == 10;
  })
    ;
  }
  ...
  onGreeterClick() {
    ...
    await
    this.request('get', '/greeter', {}, errorData = > {
    ...
    await
    this.request('get', '/greeter', {name: 'petike'}, errorData = > {
    ...
    await
    this.request('get', '/greeter', {name: 'petike', title: 'student'}, data =
    > {
  ...
  onAppendaClick() {
    ...
    await
    this.request('get', '/appenda', {}, (_, error) = > {
    ...
    // expected success with specific response
    await
    this.request('get', '/appenda/kuty', {}, data = > {
  ...
  onDoUntilClick() {
    // expected success with specific response
    await
    this.request('post', '/dountil/sum', {until: 7}, data = > {
    ...
    // expected success with specific response
    await
    this.request('post', '/dountil/factor', {until: 4}, data = > {
